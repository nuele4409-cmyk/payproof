...
export async function createReservedAccount({ userId, name, contact, bvn }) {
  ...
  if (isSandbox() && !process.env.FORCE_REAL_MONNIFY) {
    const simAccount = {
      bank:   'Wema Bank',
      number: String(2000000000 + Math.floor(Math.random() * 8000000000)).padStart(10, '0'),
      name:   `PayProof — ${name}`,
    };
    logger.info('Sandbox simulated reserved account', { userId, simAccount });
    return simAccount;
  }
  ...
  if (process.env.FORCE_REAL_MONNIFY) {
    logger.info('Monnify raw createReservedAccount response', {
      userId,
