...
export function suggestionsFor(order) {
  ...
  if (idx < 5) s.push("When does Ada get paid?");
...
export function answerFor(order, question) {
  ...
  if (/(money|settle|payout|get paid|paid out|release|funds|account)/.test(q)) {
    if (order.state === "Completed") {
      return `This order settled on ${formatDateTime(order.timestamps["Completed"])}. ${naira(order.amount)} was released to Ada’s locked settlement account (GTBank ••••1234).`;
    }
    if (idx >= 1) {
      return `${naira(order.amount)} is held by Monnify right now. It settles to Ada’s locked account (GTBank ••••1234) automatically the moment the buyer confirms delivery — neither side can move it before then.`;
    }
    return `Nothing has been paid yet. Once the transfer is confirmed, ${naira(order.amount)} is held until delivery is confirmed, then settles to Ada automatically.`;
  }
  ...
  if (/(ship|deliver|track|arriv|where|courier)/.test(q)) {
    ...
    if (order.state === "Shipped") {
      return `Ada marked this as shipped on ${formatDateTime(order.timestamps["Shipped"])}. When it arrives, confirm delivery — that’s what releases her funds.`;
    }
    if (idx >= 1) {
      return `Payment is confirmed and Ada has been asked to ship. You’ll see this timeline move to Shipped when she marks it.`;
    }
  ...
  if (/(confirm.*deliver|deliver.*confirm)/.test(q)) {
    return `Confirming delivery is the final step: it releases ${naira(order.amount)} from the Monnify hold to Ada’s settlement account and marks the order Completed. Only do it once the item is in your hands.`;
  }
