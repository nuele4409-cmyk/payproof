...
export async function createOrder({ buyerName, sellerId, productSlug, phone }, client = db) {
...
export async function findPendingOrderBySeller(sellerId, amount, client = db) {
  // Prefer the pending order whose amount matches the payment, since a seller
  // shares one NUBAN across every listing. Falls back to most-recent-pending
  // when nothing matches so the webhook still lands somewhere reviewable.
  if (typeof amount === 'number' && amount > 0) {
    const exact = await client.order.findFirst({
      where:   { sellerId, state: 'Pending Payment', amount },
      orderBy: { createdAt: 'desc' },
    });
